...
class HexParserException(Exception):
	""" exeption class for the Intel-Hex-Parser """
	pass

class Segment:
	""" holds a string joined to its start address """
...
class IntelHexParser:
	...
	def load_file(self, filename):
		""" reads the file to an internal buffer """
		try:
			file = open(filename)
	...
	def load_hex_data(self, file):
		""" reads from a file in Intel hex format """
		segmentdata	= []
		currentAddr	= 0
		startAddr	= 0
...
# a short usage example for the Intel hex parser
#
# reads the Intel hex file given as an argument and prints the processed contents
